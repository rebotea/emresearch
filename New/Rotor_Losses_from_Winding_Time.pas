unit Rotor_Losses_from_Winding_Time;
...
procedure Eta_values(mu0,cond_m,w,k,j,cond_sl:Extended; out eta_m,eta_s:extended);
 procedure surfACE(eta_m,hm,j,k,H_sl,eta_s,alpha_m,g_act,wi,mu0 :Extended; out alpha,alpha_s,alpha_f,top1,bot1,top2,bot2,Zs:extended) ;
...
begin
eta_m:= sqrt((j*mu0*cond_m)*w + (k*k));
eta_s:= sqrt((j*mu0*cond_sl)*w + (k*k));
end;
...
top1:= (j*(k/eta_s)*sin(eta_s*h_sl)) + (alpha_m*cos(eta_s*h_sl));
bot1 := (j*(k/eta_s)*cos(eta_s*h_sl)) + (alpha_m*sin(eta_s*h_sl));
alpha_s := j*(k/eta_s)*(top1/bot1);
top2 := (j*sin(k*g_act)) + (alpha_s*cos(k*g_act));
bot2 :=  (j*cos(k*g_act)) + (alpha_s*sin(k*g_act));
alpha_f := j*(top2/bot2);
Zs := (mu0*wi/k)*alpha_f;

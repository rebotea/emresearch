unit RotorLossesFromSlotEffectsUnit;
...
procedure Input_Titanium_sleeve_thickness_based_on_stress_results(titan_str,ttitan,t:Extended;stop:integer; out dummy:extended );
...
procedure Input_Inconel_sleeve_thickness_based_on_stress_results(inconel_str,tlnconel,t:Extended;stop:integer; out dummy:extended );
procedure Calculate_can_losses(p,rpm, ksm, am, lst, rr, magnet_res ,T_stain ,Ttitan, tcarfib, tlnconel, stain_res, titan_res, carfib_res, inconel_res, r, hm, b, pi :Extended; out w_magnet,p_magnet,w_stain,w_carfib,W_titan,w_inconel:extended );
...
begin
  ...
  w_Titan := (sqr(pi)/3600)*(sqr(B *rpm*(Rr+hM))*tTitan)/Titan_res;
  w_CarFib :=(sqr(pi)/3600)*(sqr(B *rpm*(Rr+hM))*tCarFib)/CarFib_res;
  w_Inconel :=(sqr(pi)/3600)*(sqr(B*rpm*(Rr+hm))*tlnconel)/Inconel_res;
  for k:= 1 to 10 do
    begin
